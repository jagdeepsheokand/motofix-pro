...
const VehicleTable = ({ vehicles, onEdit, onDelete }) => {
  ...
  return (
    <div className="w-full max-w-full overflow-hidden"> {/* Added max-w-full */}
      {/* Desktop Grid View - Cards in a grid */}
      <div className="hidden md:grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
        {vehicles.map((vehicle) => (
          <div
            key={vehicle._id}
            className="group bg-slate-800/30 border border-slate-700/30 rounded-xl p-5 hover:border-orange-500/30 hover:bg-slate-800/50 transition-all duration-300 hover:shadow-lg hover:shadow-orange-500/5 hover:translate-y-[-2px] overflow-hidden"
          >
            {/* Header with Avatar and Vehicle Info */}
            <div className="flex items-start justify-between mb-4 gap-2"> {/* Added gap-2 */}
              <div className="flex items-center gap-3 min-w-0 flex-1">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-orange-500/20 to-orange-600/10 flex items-center justify-center text-orange-400 font-semibold text-lg border border-orange-500/20 flex-shrink-0">
                  {vehicle.brand?.charAt(0).toUpperCase() || '?'}
                </div>
                <div className="min-w-0 flex-1">
                  <h3 className="font-semibold text-white group-hover:text-orange-400 transition-colors duration-200 truncate">
                    {vehicle.brand} {vehicle.model}
                  </h3>
                  <p className="text-sm text-zinc-400 truncate">{vehicle.owner?.name || 'No owner'}</p>
                </div>
              </div>
              
              {/* Status Badge */}
              {vehicle.status && (
                <span className={`px-2.5 py-1 rounded-full text-xs font-medium flex-shrink-0 ${getStatusColor(vehicle.status)}`}>
                  {vehicle.status}
                </span>
              )}
            </div>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex items-start gap-2">
                ...
                <span className="text-zinc-400 font-mono break-all overflow-wrap-anywhere min-w-0">
              ...
              {vehicle.owner?.phone && (
                <div className="flex items-start gap-2">
                  ...
                  <span className="text-zinc-300 break-all min-w-0">{vehicle.owner.phone}</span>
                </div>
              )}
              ...
              {vehicle.vin && (
                <div className="flex items-start gap-2">
                  ...
                  <span className="text-zinc-400 text-xs font-mono break-all min-w-0">VIN: {vehicle.vin}</span>
                </div>
              )}
            </div>
      ...
      <div className="md:hidden divide-y divide-slate-700/30">
        {vehicles.map((vehicle) => (
          <div key={vehicle._id} className="p-4 hover:bg-slate-800/20 transition-colors overflow-hidden">
              ...
              <div className="mt-3 pt-3 border-t border-slate-700/30 space-y-1.5">
                {vehicle.registrationNumber && (
                  <div className="flex items-start gap-2 text-xs">
                    ...
                    <span className="text-zinc-400 font-mono break-all overflow-wrap-anywhere min-w-0">{vehicle.registrationNumber}</span>
                  </div>
                )}
                ...
                {vehicle.owner?.phone && (
                  <div className="flex items-start gap-2 text-xs">
                    ...
                    <span className="text-zinc-400 break-all min-w-0">{vehicle.owner.phone}</span>
                  </div>
                )}
              </div>
            )}
